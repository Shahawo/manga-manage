...
export function applyCoreMixin(app) {
    ...
    // ─── AI VISION: QUÉT ẢNH BÌA ────────────────────────────────────────────
    _aiScanImageDataUrl: null,
    ...
    async runCoverOcr() {
        ...
        try {
            ...
            await this.searchCatalogByOcrText(text);
        } catch (err) {
            console.error('[AI Vision] OCR error:', err);
            if (processingEl) processingEl.style.display = 'none';
            if (retryBtn) retryBtn.style.display = 'flex';
            this.showToast('Lỗi nhận diện ảnh. Thử lại với ảnh rõ hơn!', 'error');
        }
    },

    async searchCatalogByOcrText(rawText) {
        const processingEl = document.getElementById('ai-scan-processing');
        const retryBtn = document.getElementById('ai-scan-retry-btn');
        ...
        try {
            ...
            if (words.length === 0) {
                if (processingEl) processingEl.style.display = 'none';
                if (retryBtn) retryBtn.style.display = 'flex';
                this.showToast('Không đọc được chữ từ ảnh. Thử chụp rõ hơn!', 'error');
                return;
            }
            ...
            if (results.length === 0) {
                this.showToast('Không tìm thấy sách phù hợp trong kho. Hãy nhập thủ công!', 'info');
                document.getElementById('ai-scan-results').style.display = 'none';
                return;
            }

            // Hiển thị kết quả để chọn
            this._showAiScanResults(results);
        } catch (err) {
            console.error('[AI Vision] Search error:', err);
            if (processingEl) processingEl.style.display = 'none';
            if (retryBtn) retryBtn.style.display = 'flex';
            this.showToast('Lỗi tìm kiếm catalog. Vui lòng thử lại!', 'error');
        }
    },

    _showAiScanResults(results) {
        ...
        if (window.feather) { try { feather.replace(); } catch(e) {} }
        this.showToast(`Tìm thấy ${results.length} kết quả — chọn cuốn phù hợp!`, 'info');
    },
