...
const PIE_COLORS = ['#16a34a', '#2563eb', '#7c3aed', '#ea580c', '#0891b2', '#db2777', '#d97706', '#64748b'];
...
function _getBrandColor(name, fallbackIdx) {
    return PUBLISHER_BRANDS[name]?.color ?? PIE_COLORS[fallbackIdx % PIE_COLORS.length];
}
...
function _populatePeriodDropdown(tf) {
    ...
    menu.innerHTML = sorted.map(key => {
        const label = _periodLabel(tf, key);
        const isActive = key === _currentPeriod ? 'style="font-weight:700; color:var(--primary);"' : '';
...
function _renderSummaryCards() {
    ...
    const newSeriesThisMonth = Object.values(seriesFirstDate).filter(d =>
...
function _renderMonthlyMessage() {
    ...
    const newSeriesCount = Object.values(seriesFirstDate).filter(d =>
        d.getMonth() === thisMonth && d.getFullYear() === thisYear
    ).length;
...
function _buildBarData(tf, period) {
    ...
    if (tf === 'quarter') {
        ...
        const qNum = parts[1] ? parseInt(parts[1].replace('Q', '')) : Math.floor(new Date().getMonth() / 3) + 1;
        const startM = (qNum - 1) * 3; // 0-indexed first month of quarter

        const monthNames = ['Tháng 1', 'Tháng 2', 'Tháng 3', 'Tháng 4', 'Tháng 5', 'Tháng 6',
            'Tháng 7', 'Tháng 8', 'Tháng 9', 'Tháng 10', 'Tháng 11', 'Tháng 12'];
        const labels = [monthNames[startM], monthNames[startM + 1], monthNames[startM + 2]];
    ...
    _pieChart = new Chart(canvas, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: ctx => ` ${ctx.label}: ${ctx.parsed} cuốn (${Math.round(ctx.parsed / total * 100)}%)`
                    }
                }
            }
        }
