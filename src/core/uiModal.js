...
export function toggleModalImage(dir) {
  if (
    !window.app.currentModalImages ||
    window.app.currentModalImages.length < 2
  )
    return;
  window.app.currentModalImageIndex =
    (window.app.currentModalImageIndex +
      dir +
      window.app.currentModalImages.length) %
    window.app.currentModalImages.length;
  document.getElementById("modal-cover-img").src =
    window.app.currentModalImages[window.app.currentModalImageIndex];
}
...
export function editVolume(id) {
  ...
  window.app.navigateTo("/form?id=" + id);
  populateEditForm(id);
}
...
export function populateEditForm(id) {
  const manga = store.data.find((m) => m.id === id);
  if (!manga) {
    window.app.showToast("Không tìm thấy thông tin sách để sửa.", "error");
  ...
  setVal(
    "price",
    manga.price ? new Intl.NumberFormat("vi-VN").format(manga.price) : "",
  );
  ...
  if (giftsArray.length > 0)
    window.app.previewGiftImage(giftsArray[0], "main-");
...
export async function deleteVolume(id) {
  ...
  const isSearchView = document
    .getElementById("view-search")
