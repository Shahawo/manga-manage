...
export async function init() {
    ...
    if (!document.hidden) {
      ...
      if (loadingOverlay && loadingOverlay.style.display === "flex") {
        ...
        if (Date.now() - startTime > 30000) {
          // Nếu kẹt quá 30 giây (có thể do ngủ đông mạng)
          if (window.app.hideLoading) window.app.hideLoading();
          if (window.app.showToast)
            window.app.showToast(
              "Tác vụ có thể bị gián đoạn. Vui lòng thử lại!",
              "error",
            );
        }
      }
      ...
      const scheduleLoadingBar = document.getElementById(
        "schedule-loading-bar",
      );
      const scheduleBody = document.getElementById("schedule-body");
      const isScheduleLoading =
        store.currentView === "schedule" &&
        ((scheduleLoadingBar && scheduleLoadingBar.style.opacity === "1") ||
          (scheduleBody &&
            scheduleBody.innerHTML.includes("schedule-loading")));
      ...
      if (store.currentView === "admin") {
        if (
          !store.fullCatalogCache ||
          document.getElementById("admin-catalog-loading")
        ) {
          setTimeout(
            () =>
              window.app.searchAdminCatalog && window.app.searchAdminCatalog(1),
            500,
          );
        }
        if (document.getElementById("admin-pending-loading")) {
          setTimeout(
            () =>
              window.app.fetchPendingBooks && window.app.fetchPendingBooks(),
            500,
          );
        }
      }
  ...
  window.app._startSyncInterval = () => {
    ...
    window.app._syncIntervalId = setInterval(
      () => window.app.processSyncQueue(),
      10000,
    );
  };
...
export function updateThemeIcon(theme) {
  ...
  if (themeSwitchSettings)
    themeSwitchSettings.classList.toggle("active", theme === "dark");
  if (window.feather) {
    try {
      feather.replace();
    } catch (e) {
