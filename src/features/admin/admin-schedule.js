...
function _adminScheduleUpdateLabel() {
  ...
  if (el)
    el.textContent = `Tháng ${_adminScheduleState.month}/${_adminScheduleState.year}`;
}
...
export async function adminScheduleLoad() {
      ...
      .fill(
        `
            <div style="display:flex;align-items:center;gap:0.75rem;padding:0.75rem;border:1px solid var(--border);border-radius:8px;background:var(--surface);">
                <div class="skeleton" style="width:36px;height:52px;border-radius:4px;flex-shrink:0;"></div>
                <div style="flex:1;min-width:0;">
                    <div class="skeleton skeleton-text medium" style="margin-bottom:0.4rem;"></div>
                    <div class="skeleton skeleton-text short" style="margin-bottom:0; height:0.8rem;"></div>
                </div>
  ...
  try {
    const res = await window.app.executeWithAbort(
      () =>
        window.app.apiFetch(
          `/api/admin/schedule?start=${firstDay}&end=${lastDay}`,
        ),
      5000,
    ...
    if (list)
      list.innerHTML = `<p style="text-align:center;color:var(--danger);padding:2rem;">Lỗi: ${err.message}</p>`;
  }
}
...
export function renderAdminReleaseList(entries) {
  ...
  if (!entries.length) {
    list.innerHTML =
      '<p style="text-align:center;color:var(--text-muted);padding:2rem;">Chưa có entry nào trong tháng này.</p>';
    return;
  }
        ...
        <div style="display:flex;flex-direction:column;gap:0.5rem;">
            ${entries
              .map((e) => {
                ...
                const dateStr = e.release_date
                  ? new Date(e.release_date + "T00:00:00").toLocaleDateString(
                      "vi-VN",
                    )
                  : "?";
                const volText = e.volume
                  ? ` — Tập ${e.volume % 1 === 0 ? parseInt(e.volume) : e.volume}`
                  : "";
                const priceText = e.price
  ...
  if (id) {
    ...
    window.app
      ...
      .then((res) => {
        ...
        document.getElementById("admin-release-cover").value =
          data.cover_url ?? "";
        document.getElementById("admin-release-edition").value =
...
export async function saveAdminRelease() {
  ...
  const series =
    document.getElementById("admin-release-series").value.trim() || null;
  const volume =
    parseFloat(document.getElementById("admin-release-volume").value) || null;
  const publisher =
    document.getElementById("admin-release-publisher").value.trim() || null;
  const price =
    parseInt(document.getElementById("admin-release-price").value) || null;
  const cover_url =
    document.getElementById("admin-release-cover").value.trim() || null;
  const edition = document.getElementById("admin-release-edition").value;
  const note =
    document.getElementById("admin-release-note").value.trim() || null;

  if (!release_date) {
    window.app.showToast("Vui lòng chọn ngày phát hành!", "error");
    return;
  }
  if (!title) {
    window.app.showToast("Vui lòng nhập tiêu đề sách!", "error");
    return;
  }
  ...
  try {
    ...
    const res = await window.app.executeWithAbort(
      () =>
        window.app.apiFetch("/api/admin/import", {
          method: "POST",
    ...
    if (result?.error === "duplicate") {
      window.app.showToast(
        "Entry trùng lặp (title + ngày đã tồn tại)!",
        "error",
      );
      return;
    }
