...
export async function startBarcodeScanner() {
  ...
  try {
    ...
    store.codeReader.decodeFromVideoDevice(
      selectedDeviceId,
      "scanner-video",
      (result, err) => {
        if (result) {
      ...
      store.codeReader
        ...
        .catch((err) => {
          ...
          window.app.showToast(
            "Lỗi: Hình bị mờ hoặc không có mã ISBN hợp lệ. Hãy chụp rõ ràng!",
            "error",
          );
        });
    };
  ...
  img.onload = () => {
    ...
    store.codeReader
      ...
      .catch((err) => {
        console.error(err);
        window.app.showToast(
          "Không bắt được Barcode nào, vui lòng chụp lại rõ hơn!",
          "error",
        );
      });
  };
...
export async function onBarcodeDetected(isbn) {
  ...
  try {
    const res = await window.app.apiFetch(
      `/api/catalog/search?isbn=${encodeURIComponent(isbn)}`,
    );
    const data = res.data;
    if (data && data.length > 0) {
      ...
      window.app.showToast(
        "Mã ISBN chưa có trong kho chung. Vui lòng điền thông tin để đóng góp!",
        "info",
      );
...
export function showBookPreview(catalogBook) {
  ...
  document.getElementById("preview-title").textContent =
    catalogBook.title || catalogBook.series || "Chưa rõ";
  document.getElementById("preview-author").textContent =
    catalogBook.author || "Đang cập nhật";
  document.getElementById("preview-publisher").textContent =
    catalogBook.publisher || "-";
  document.getElementById("preview-isbn").textContent = catalogBook.isbns
    ? catalogBook.isbns[0]
    : "";
...
export function applyBookToForm() {
  ...
  if (book.gift_urls && book.gift_urls.length > 0) {
    ...
    window.app.previewGiftImage(
      book.gift_urls[book.gift_urls.length - 1],
      "main-",
    );
  }
...
export async function runCoverOcr() {
  ...
  try {
    ...
    await window.app.loadLibrary(
      "https://unpkg.com/tesseract.js@v5/dist/tesseract.min.js",
      "Tesseract",
    );
    ...
    const {
      data: { text },
    } = await Tesseract.recognize(store.aiScanImageDataUrl, "vie+eng", {
      logger: (m) => {
...
export async function searchCatalogByOcrText(rawText) {
  ...
  try {
    ...
    if (words.length === 0) {
      ...
      if (retryBtn) retryBtn.style.display = "flex";
      window.app.showToast(
        "Không đọc được chữ từ ảnh. Thử chụp rõ hơn!",
        "error",
      );
      return;
    }
    ...
    const searchPromises = searchPhrases
      .slice(0, 6)
      .map((phrase) =>
        window.app.apiFetch(
          `/api/catalog/search?q=${encodeURIComponent(phrase)}`,
        ),
      );
...
export function _showAiScanResults(results) {
  ...
  results.forEach((book) => {
    ...
    div.innerHTML = `
                ${
                  book.cover_url
                    ? `<img src="${book.cover_url}" class="ai-result-cover" onerror="this.style.display='none'">`
  ...
  window.app.showToast(
    `Tìm thấy ${results.length} kết quả — chọn cuốn phù hợp!`,
    "info",
  );
}
