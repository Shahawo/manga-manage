...
export function switchImgTab(tabId, prefix = "main-") {
  ...
  const coverTabBtn = document.querySelector(
    `[onclick="app.switchImgTab('cover', '${prefix}')"]`,
  );
  const giftTabBtn = document.querySelector(
    `[onclick="app.switchImgTab('gift', '${prefix}')"]`,
  );
  ...
  const btn = document.querySelector(
    `[onclick="app.switchImgTab('${tabId}', '${prefix}')"]`,
  );
...
export function previewImage(url, type = "cover", prefix = "main-") {
  const boxId =
    type === "cover"
      ? `${prefix}cover-preview-box`
      : `${prefix}gift-preview-box`;
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        if (supportsWebP) {
          canvas.toBlob(
            (blob) => resolve({ blob, format: "webp" }),
            "image/webp",
            quality,
          );
        } else {
          canvas.toBlob(
            (blob) => resolve({ blob, format: "jpeg" }),
            "image/jpeg",
            quality,
          );
        }
      ...
      img.onerror = () =>
        reject(new Error("Không thể đọc file ảnh này. File có thể bị lỗi."));
    };
...
export async function handleFileUpload(inputElem, type, prefix = "main-") {
  ...
  try {
    ...
    const fileName = `${safeName}_${hash}.${fileExt}`;
    ...
    const timeout = (ms) =>
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error("Yêu cầu tải ảnh quá hạn (Timeout)")),
          ms,
        ),
      );
    ...
    console.debug(
      `[Upload] Ảnh bìa đã upload dạng ${format.toUpperCase()}: ${filePath}`,
    );
  } catch (e) {
    console.error("Lỗi tải ảnh:", e);
    window.app.showToast(
      "Lỗi tải ảnh lên server: " + (e.message || ""),
      "error",
    );
  } finally {
...
export async function handleGiftFileUpload(inputElem, prefix = "main-") {
  ...
  try {
    ...
    for (let i = 0; i < inputElem.files.length; i++) {
      ...
      console.debug(
        `[Upload] Ảnh quà tặng đã upload dạng ${format.toUpperCase()}: ${filePath}`,
      );
    }
    ...
    window.app.showToast(
      "Lỗi tải ảnh quà tặng lên server: " + (e.message || ""),
      "error",
    );
  } finally {
    window.app.hideLoading();
...
export function removeGiftUrl(index, prefix = "main-") {
  ...
  window.app.renderGiftThumbnails(prefix);
  if (urls.length > 0)
    window.app.previewGiftImage(urls[urls.length - 1], prefix);
...
export function renderGiftThumbnails(prefix = "main-") {
  ...
  urls.forEach((url, index) => {
    ...
    wrap.style.cssText =
      "position:relative; width:60px; height:80px; border-radius:6px; overflow:hidden; border:2px solid var(--border); flex-shrink:0; cursor:grab; user-select:none; transition: opacity 0.2s, transform 0.15s;";
    wrap.draggable = true;
...
export function previewManyImages() {
  window.app.renderGiftThumbnails();
}
...
export async function handleFormSubmit(e) {
  ...
  try {
    ...
    const mangaData = {
      ...
      gift_urls: getVal("main-giftUrls")
        .split("\n")
        .map((s) => s.trim())
        .filter((s) => s !== ""),
    };
    ...
    if (isRegularEdition) {
      const isDuplicate = store.data.some(
        (m) =>
      ...
      if (isDuplicate) {
        window.app.showToast(
          `Tập ${mangaData.volume} (Bản thường) đã tồn tại trong series này!`,
          "error",
        );
        return;
      }
    ...
    if (err.name === "AbortError") {
      window.app.showToast(
        "Form đã được lưu offline. Quay lại tab để tiếp tục!",
        "info",
      );
    } else {
      window.app.showToast(
        "Lỗi khi lưu: " + (err.message || "unknown"),
        "error",
      );
    }
  }
...
export function setupPriceInput() {
  ...
  priceInput.addEventListener("blur", function () {
    ...
    if (num > 2000000000) num = 2000000000; // Ngăn lỗi D1 Database (Integer limit)
...
export async function autoFill() {
  ...
  if (!seriesName) {
    window.app.showToast(
      "Vui lòng nhập Tên Series rồi nhấn Tự động điền!",
      "error",
    );
    return;
  }
  ...
  try {
    const res = await window.app.apiFetch(
      `/api/catalog/search?q=${encodeURIComponent(seriesName)}`,
    );
    const data = res.data || [];
    if (data && data.length > 0) {
      const resData = data[0];
      if (resData.author)
        document.getElementById("author").value = resData.author;
      if (resData.translator)
        document.getElementById("translator").value = resData.translator;
      if (resData.publisher)
        document.getElementById("publisher").value = resData.publisher;
      if (resData.distributor)
        document.getElementById("distributor").value = resData.distributor;
      if (resData.size) document.getElementById("size").value = resData.size;
      if (resData.price) {
        document.getElementById("price").value = new Intl.NumberFormat(
          "vi-VN",
        ).format(resData.price);
      }
      ...
      window.app.showToast(
        "Chưa có dữ liệu tham khảo cho Series này trong Kho chung.",
        "info",
      );
    }
  } catch (err) {
...
export function renderSearch(query) {
  ...
  const matchedItems = store.data.filter((m) => {
    ...
    const matchIsbn =
      mIsbnStr && qIsbnStr.length >= 6 && mIsbnStr.includes(qIsbnStr);
    ...
    const searchable =
      `${m.title || ""} ${m.series || ""} ${m.volume ? "tập " + m.volume : ""} ${m.author || ""} ${m.translator || ""}`.toLowerCase();
    const matchText =
      queryWords.length > 0 && queryWords.every((w) => searchable.includes(w));

    return (
      matchText || matchIsbn || (m.isbn && m.isbn.toLowerCase().includes(query))
    );
  });
  ...
  if (matchedItems.length === 0) {
    list.className = "";
    list.innerHTML =
      '<p style="text-align:center;color:var(--text-muted);padding:3rem;">Không tìm thấy kết quả nào.</p>';
  ...
  if (hasMultipleIsbnMatch) {
    ...
    note.style.cssText =
      "grid-column: 1 / -1; background: #fffbeb; color: #b45309; padding: 0.75rem 1rem; border-radius: 8px; font-size: 0.9rem; border: 1px solid #fde68a; display: flex; align-items: center; gap: 0.4rem;";
  ...
  matchedItems.forEach((v) => {
    const coverUrl =
      v.coverUrl || "https://via.placeholder.com/200x300.png?text=No+Cover";
