...
const EDITION_MAP = { '': 'standard', standard: 'standard', special: 'special', collector: 'collector', limited: 'limited' };
...
export function toggleImportPanel() {
    ...
    if (!isHidden) {
        ...
        if (monthInput && !monthInput.value) {
            ...
            const label = document.getElementById('admin-schedule-month-label')?.textContent ?? '';
...
function _renderPreviewRow(row, index, prefix) {
    const volText = row.volume ? `Tập ${row.volume % 1 === 0 ? parseInt(row.volume) : row.volume}` : '—';
    const priceText = row.price ? `${Number(row.price).toLocaleString('vi-VN')}đ` : '—';
    const dateText = row.release_date ?? '?';
    const editionLabel = { special: '🔶 Đặc Biệt', collector: '🔷 Sưu Tầm', limited: '💜 Giới Hạn' }[row.edition] ?? '';
...
export async function fetchFromTana() {
    ...
    try {
        ...
        while (page <= totalPages) {
            const filter = encodeURIComponent(`publishDate >= '${from}' && publishDate <= '${to}'`);
            const expand = encodeURIComponent('publication.release.title,publication.release.publisher,assets_via_book,publication.assets_via_publication');
        ...
        _tanaRows = allItems.map(item => {
            ...
            if (item.expand?.assets_via_book?.length > 0) {
                asset = item.expand.assets_via_book.sort((a, b) => a.priority - b.priority)[0];
            } else if (pub?.expand?.assets_via_publication?.length > 0) {
                asset = pub.expand.assets_via_publication.sort((a, b) => a.priority - b.priority)[0];
            }
            ...
            const release_date = item.publishDate ? item.publishDate.split(' ')[0] : null;
        ...
        // Render preview
        previewTable.innerHTML = _tanaRows.map((r, i) => _renderPreviewRow(r, i, 'tana')).join('');
...
export function tanaUpdateCount() { _updateCount('tana'); }

export function tanaToggleAll(checked) {
    document.querySelectorAll('.tana-row-check').forEach(cb => cb.checked = checked);
    _updateCount('tana');
}

export async function importSelectedTana() {
    const checked = [...document.querySelectorAll('.tana-row-check:checked')];
    if (!checked.length) { window.app.showToast('Chưa chọn entry nào!', 'error'); return; }
...
const CSV_HEADERS = ['release_date', 'series', 'title', 'volume', 'publisher', 'price', 'cover_url', 'edition', 'note'];

function _parseCsv(text) {
    const lines = text.trim().split(/\r?\n/);
    if (lines.length < 2) throw new Error('CSV cần ít nhất 2 dòng (header + data)');

    const header = lines[0].split(',').map(h => h.trim().toLowerCase());
    return lines.slice(1).map((line, li) => {
        if (!line.trim()) return null;
        // Simple CSV split (không hỗ trợ quoted commas — đủ cho use case này)
        const vals = line.split(',');
        const obj = {};
        header.forEach((h, i) => { obj[h] = vals[i]?.trim() ?? null; });
        if (!obj.title) throw new Error(`Dòng ${li + 2}: thiếu cột 'title'`);
        if (!obj.release_date) throw new Error(`Dòng ${li + 2}: thiếu cột 'release_date'`);
...
function _parseJson(text) {
    ...
    return arr.map((item, i) => {
        ...
        if (!item.release_date) throw new Error(`Item [${i}]: thiếu trường 'release_date'`);
        return {
...
export function parseCsvImport() {
    ...
    if (!input) { window.app.showToast('Paste dữ liệu vào trước!', 'error'); return; }
    _csvRows = [];
    ...
    try {
        ...
        previewTable.innerHTML = _csvRows.map((r, i) => _renderPreviewRow(r, i, 'csv')).join('');
...
export function csvUpdateCount() { _updateCount('csv'); }
