 ...
 * Data source: Cloudflare D1 release_calendar
 */
...
async function _fetchAndRender(retryCount = 0) {
  ...
  try {
    const res = await window.app.withTimeout(
      window.app.apiFetch(`/api/schedule?from=${firstDay}&to=${lastDay}`, {
        signal: calendarState.fetchController.signal,
      }),
      8000,
    ...
    if (retryCount < 1 && err.message.includes("quá hạn")) {
      console.warn(
        "[schedule] Yêu cầu bị kẹt (thường do chuyển tab), tự động thử lại...",
      );
...
function _render(releases) {
  ...
  const html = calendarState.releaseDates
    .map((dateStr) => {
      ...
      const cardsHtml = books
        .map((b) => _renderCard(b, userSeriesMap))
        .join("");
...
function _renderError(msg) {
  const body = document.getElementById("schedule-body");
  if (body)
    body.innerHTML = `<div class="schedule-empty"><i data-feather="alert-circle" style="width:48px;height:48px;color:var(--danger);margin-bottom:1rem;"></i><h3>Lỗi tải dữ liệu</h3><p style="color:var(--text-muted);">${_esc(msg)}</p></div>`;
  if (window.feather) feather.replace();
}
...
function _buildPublisherFilter(releases) {
  const publishers = [
    ...new Set(releases.map((r) => r.publisher).filter(Boolean)),
  ].sort();
  ...
  if (publishers.length === 0) {
    dropdown.innerHTML =
      '<div style="padding:0.75rem 1rem;color:var(--text-muted);font-size:0.85rem;">Không có dữ liệu</div>';
    return;
  }
...
export function scheduleSelectPublisher(publisher) {
  ...
  if (label) {
    label.textContent =
      calendarState.selectedPublishers.length === 0
        ? "Chọn nhà xuất bản"
  ...
  document.querySelectorAll(".schedule-publisher-item").forEach((btn) => {
    ...
    const isActive = pub
      ? calendarState.selectedPublishers.includes(pub)
      : calendarState.selectedPublishers.length === 0;
...
function _updateMonthLabel() {
  ...
  if (el)
    el.textContent = `${MONTHS_VI[calendarState.month - 1]}/${calendarState.year}`;
}
...
function _buildYearSelect() {
  ...
  sel.innerHTML = years
    .map(
      (y) =>
        `<option value="${y}" ${y === calendarState.pickerYear ? "selected" : ""}>${y}</option>`,
    )
    .join("");
}
...
function _highlightNearestDay() {
  ...
  const isCurrentMonth =
    ...
    today.getMonth() + 1 === calendarState.month;
...
function _onScroll() {
  ...
  const visible = Array.from(els).filter(
    (el) => el.getBoundingClientRect().bottom > 100,
  );
  if (visible.length > 0) {
